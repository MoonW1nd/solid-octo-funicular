...
> Так же можете посмотреть получившийся результат на [Heroku](https://shrouded-reef-93129.herokuapp.com/).
...
В некоторых местах замени `var` на `const`, чтобы все объявление переменных были в едином стиле ES5. Так как не было никаких настроек в зависимостях `packaje.json`. То понял что по код стайлу нет никаких ограничений. Для обеспечения единого стиля кода прогнал все файлы через `Prettier`.
...
### Дополнительно
  ...
  - Переписал код под express и запустил сайт на [Heroku](https://shrouded-reef-93129.herokuapp.com/).
